...
index_statements = np.array(cursor.execute(
    """
    SELECT tbl_name, sql FROM sqlite_master WHERE type='index' AND name='%s'
    """%TABLE_NAME).fetchall()
)
...
keep_items_SQL = "SELECT %%s FROM %s WHERE %s"%(TABLE_NAME, " OR ".join(
    "item_id=%s"%item for item in [item for item in unique_items if item not in unique_aggregate_items])
    )
...
for key, values in aggregate_item_lookup.iteritems():
    ...
    key_item_SQL = "item_id=%s AND element_id=51"%key
    if key_tracker:
        key_item_SQL += " AND NOT (%s)"%" OR ".join(
            "country_id=%s"%country for country in key_tracker
        )
